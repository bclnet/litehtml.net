...
namespace litehtml
{
    ...
    public partial class element : Element
    {
        ...
        public bool in_normal_flow => get_element_position() != element_position.absolute && get_display != style_display.none;
        ...
        public bool is_positioned => get_element_position() > element_position.@static;
        ...
        public void apply_relative_shift(int parent_width)
        {
            if (get_element_position(out var offsets) == element_position.relative)
            {
                ...
                {
                    var h = 0;
                    if (offsets.bottom.units == css_units.percentage)
                    {
                        var el_parent = parent();
                        if (el_parent != null)
                            el_parent.get_predefined_height(out h);
        ...
        public element_position get_element_position() => get_element_position(out var junk);
        ...
        public object get_font() => get_font(out var junk);
        ...
        public virtual void get_redraw_box(ref position pos, int x = 0, int y = 0, int z = 0) //:h3ml
        {
            if (is_visible)
            {
                ...
                var p_front = Math.Max(pos.front, z + _pos.front + _padding.front + _borders.front); //:h3ml
                var p_back = Math.Max(pos.back, z + _pos.back + _padding.back + _borders.back); //:h3ml
