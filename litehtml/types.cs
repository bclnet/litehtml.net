...
namespace litehtml
{
    ...
    public partial struct position : Rect
    {
        ...
        public position(ref point p, ref size sz)
        {
            x = p.x;
            y = p.y;
            width = sz.width;
        ...
        public point p => new(x, y
#if H3ML
            , z
#endif
        );
        public size sz => new(width, height
#if H3ML
        , depth
#endif
        );
        ...
        public void assign(ref point p)
        {
            x = p.x;
            y = p.y;
#if H3ML
        ...
        public void assign(ref size sz)
        {
            width = sz.width;
            height = sz.height;
#if H3ML
        ...
        public void move_to(ref point p)
        {
            x = p.x;
            y = p.y;
#if H3ML
        ...
        public bool does_intersect(ref position val)
        {
        ...
        public bool is_point_inside(ref point p) =>
            p.x >= left && p.x <= right &&
            p.y >= top && p.y <= bottom
#if H3ML
            && z >= front && z <= back
    ...
    partial class types
    {
        public const string style_display_strings = "none;block;inline;inline-block;inline-table;list-item;table;table-caption;table-cell;table-column;table-column-group;table-footer-group;table-header-group;table-row;table-row-group;inline-text;flex;inline-flex";
    }
    public enum style_display
    {
        ...
        [Description("inline-flex")] inline_flex,
    }
    ...
    {
        [Description("border-box")] border,
        [Description("padding-box")] padding,
    ...
    public struct floated_box
    {
        ...
        public floated_box()
        {
            pos = default;
            float_side = default;
            clear_floats = default;
            el = default;
        }
        public floated_box(ref floated_box val)
        {
            pos = val.pos;
    ...
    public class def_value<T>
    {
        ...
        public bool is_default => m_is_default;
        public T assign(T new_val)
        {
            m_val = new_val;
            m_is_default = false;
            return m_val;
    ...
    partial class types
    {
        public const string media_feature_strings =
#if H3ML
...
#if H3ML
        [Description("device-depth")] device_depth,
        [Description("min-device-depth")] min_device_depth,
        [Description("max-device-depth")] max_device_depth,
#endif
...
#if H3ML
        public int depth;           // (pixels) The depth of the targeted display area of the output device. For continuous media, this is the depth of the viewport including the size of a rendered scroll bar (if any). For paged media, this is the depth of the page box.
#endif
...
#if H3ML
        public int device_depth;    // (pixels) The depth of the rendering surface of the output device. For continuous media, this is the depth of the screen. For paged media, this is the depth of the page sheet size.
#endif
